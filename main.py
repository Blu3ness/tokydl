...
class AudioBook:
    def __init__(self, title, tags, tracklist, location):
        self.title = title
        self.tags = tags
...
def get_abook_properties(soup, outpath):
    ...
    return AudioBook(title=book_title, tags=tags, tracklist=tracklist,location=outputFolder)
...
def parse_url(bookURL, outpath):
    ...
    # for track in tracklist:
        if not track['name'] == 'welcome':  # Skip the welcome track
            ...
            download_file(pg, outputFolder, track['name'])
    progbar.close()
    print("\nDownload Complete!\n")
